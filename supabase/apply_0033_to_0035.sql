...
--
-- These no longer hard-depend on the `pursuits` table. If it's missing they
-- still install and simply skip the pursuit-directory row, telling you so via a
-- NOTICE. That's what caused the "relation public.pursuits does not exist"
-- error — run `supabase/seed/which_db_is_this.sql` to see what your database
-- actually has.
--
...
-- select count(*) from public.spend_categories where user_id is null;  -- expect 25
-- select name, starts_on, ends_on, pursuit_id from public.challenges;
