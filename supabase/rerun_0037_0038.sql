-- ============================================================================
-- Rerun script for migrations 0037 (iOS theme) and 0038 (flexible challenges).
--
-- You mentioned 0037 and 0038 both showed a failure message in the Supabase
-- SQL editor and you only just noticed. Here's how to think about each:
--
-- 0037 (ios theme):    OBSOLETE. Migration 0037 widened profiles.theme_check
--                      to allow 'ios' as a fourth value. iOS was subsequently
--                      demoted from a theme back to a shared feel (see
--                      src/lib/theme.ts -- ThemeName no longer includes it,
--                      normaliseTheme() coerces any legacy 'ios' back to
--                      'light'). So whether 0037 ever ran or not, the app is
--                      correct: it never tries to save 'ios' any more. No
--                      action needed.
--
-- 0038 (flexible challenges):  MATTERS. This is the migration that lets a
--                      challenge measure life hours / sleep / focus / a
--                      pursuit stat, not just non-essential dollars. Every
--                      "Create a challenge" starter template except Budget
--                      Baddies calls into it. It is written to be safe to
...
--                      ... create policy`).
--
-- So the whole plan is: run this file. It will (1) run the small piece of
-- 0037 you might not have, and (2) re-run 0038 in full. The first pass is
...
-- ---------- 0038 (idempotent) ----------------------------------------------
-- If any of the pieces below fail for you, TELL ME the exact error text --
-- it will name the missing dependency (usually is_challenge_member(),
-- can_see_pursuit(), or challenge_income()), all of which are set up by
-- migration 0035 or 0036. If 0035/0036 never ran cleanly, run them first.

-- 1. Columns and constraints on public.challenges
...
-- 4. Standings (drop then create, so the return shape can grow)
...
-- 5. Daily race chart (same signature refresh)
...
-- 7. finalize_challenge (direction-aware)
...
-- 8. my_challenges (widened return shape)
...
-- ============================================================================
-- Done. Sanity checks below -- if any of these come back empty, that piece
-- didn't take and something is wrong. Otherwise you're good.
-- ============================================================================

-- All 12 metric values are legal:
select conname, pg_get_constraintdef(oid) as def
  from pg_constraint
 where conname = 'challenges_metric_allowed';

-- All the helper functions exist with the right signatures:
