-- =============================================================================
-- Rerun of migration 0038 — flexible challenges. Idempotent.
--
-- Your DB diagnostic showed:
--   * challenges.direction / stat_id  : missing
--   * challenge_metric*() functions   : missing
--   * challenges metric check         : still the pre-0038 short list
--   * profiles.theme / accent         : present (so no self-heal needed)
--   * challenge_income() etc.         : present (so 0036 is in place, which
--                                       0038 depends on)
--
-- So this is just 0038, standalone. Every statement is either `if not
-- exists`, `create or replace`, or a drop-then-add pair — safe to run
-- more than once.
-- =============================================================================

-- 1. Columns + constraints on public.challenges
...
-- Postgres generated. Find it by contents rather than guessing.
do $$
declare cname text;
begin
  for cname in
...
alter table public.challenges add constraint challenges_metric_allowed
  check (metric in (
    -- legacy spellings kept legal so existing rows still validate
...
-- 3. RLS gates + freeze trigger + join_challenge refresh
...
-- 4. Standings (drop-then-create so the return shape can grow cleanly)
...
-- 6. Category breakdown — money only
...
-- =============================================================================
-- Done. Re-run where_am_i.sql — rows 8 and 9 should flip to YES.
-- =============================================================================
